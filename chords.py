...
def boundary_cyclic_order(square: Square) -> List[BoundaryPoint]:
    ...
    def _ordered_ports(side: Side, edge) -> Iterable[Port]:
        ...
        # Targeted flip for shared interior edges: reverse RIGHT side ordering.
        if side == Side.RIGHT and base is not None and base != side.value:
            ports = list(reversed(ports))
            return ports
        if base is not None and base != side.value:
            ports = list(reversed(ports))
        return ports
